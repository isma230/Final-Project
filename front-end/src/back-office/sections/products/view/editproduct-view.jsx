...
const EditProductPage = () => {
  ...
  const getProduct = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/v1/products/${id}`, { withCredentials: true });
        ...
        <form onSubmit={handleSubmit}>
          <Stack spacing={3} p={5}>
            ...
            <label htmlFor="upload-button">
              {formData.product_image ? formData.product_image.name : 'No file selected'}
            </label>
            <input
              hidden
              id="upload-button"
